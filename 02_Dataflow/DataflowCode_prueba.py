...
def runDataflow():
    ...
    parser.add_argument(
                '--output_bigquery_consumption',
                required=True,
                help='Table where comsupmtion will be stored in BigQuery. Format: <dataset>.<table>.')
    parser.add_argument(
                '--output_bigquery_anomalous_consumption',
                required=True,
                help='Table where anomalous_comsumption will be stored in BigQuery. Format: <dataset>.<table>.')
    parser.add_argument(
                '--output_bigquery_consumption_period',
                required=True,
                help='Table where consumption_period will be stored in BigQuery. Format: <dataset>.<table>.')
    parser.add_argument(
    ...
    parser.add_argument(
                '--bigquery_schema_path_anomalous_consumption',
                required=False,
                default='./Schemas/bq_schema_anomalous_consumption.json',
                help='BigQuery Schema Path Anomalous_consumption within the repository.')
    parser.add_argument(
                '--bigquery_schema_path_consumption_period',
                required=False,
                default='./Schemas/bq_schema_consumption_period.json',
    ...
    # Anomalous Consumption table
    with open(args.bigquery_schema_path_anomalous_consumption) as file:
        input_schema = json.load(file)

    schema = bigquery_tools.parse_table_schema_from_json(json.dumps(input_schema))

    #Consumption Period table
    with open(args.bigquery_schema_path_consumption) as file:
        input_schema = json.load(file)
